...
public class CompraLibroModel {
    ...
    public static void actualizarCompraLibro(CompraLibro compra) throws Exception {
        try (Connection cn = MySQLConexion.getConexion();
             CallableStatement st = cn.prepareCall("{CALL SP_ActualizarCompra(?, ?, ?, ?, ?, ?, ?)}")) {

        	st.setString(1, compra.getIdCompra());
        	st.setString(2, compra.getIdLibro());
        	st.setString(3, compra.getIdEditorial());
        	st.setString(4, compra.getIdEmpleado());
        	st.setDate(5, new java.sql.Date(compra.getFecCompra().getTime()));
            st.setBigDecimal(6, compra.getPrecio());
            st.setInt(7, compra.getCantidad());
